...
class Lista_en_caja(Gtk.Window):
    def __init__(self):
        ...
        etiqueta = Gtk.Label("Hora e data automatics", xaling=0)
        etiqueta2 = Gtk.Label("Require acceso a internet", xaling=0)
        caixaV_int.pack_start(etiqueta, True, True, 0)
        caixaV_int.pack_start(etiqueta2, True, True, 0)

        switch = Gtk.Switch()
        switch.props.valing = Gtk.Align.CENTER
        ...
        etiqueta3 = Gtk.Label("Permitir auto-actualizar", xaling=0)
        ...
        etiqueta4 = Gtk.Label("formato data", xaling=0)
